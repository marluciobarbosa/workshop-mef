...
class FEMSolver2D
{
  ...
  private:
    ...
    std::vector<std::vector<int>>
        elementos; // Vetor para armazenar os elementos (cada elemento é um vetor de índices de nós).
...
{
    // Geração dos nós
    nos.resize((nx + 1) * (ny + 1),
               std::vector<double>(2, 0.0)); // Redimensiona o vetor de nós para armazenar as coordenadas de cada nó.
    for (int j = 0; j <= ny; ++j)
    {
        for (int i = 0; i <= nx; ++i)
        {
    ...
    for (int j = 0; j < ny; ++j)
    {
        for (int i = 0; i < nx; ++i)
        {
            ...
            elementos.push_back(
                {n1, n3, n2}); // Primeiro triângulo: inferior esquerdo, superior esquerdo, inferior direito.
            elementos.push_back(
                {n2, n3, n4}); // Segundo triângulo: inferior direito, superior esquerdo, superior direito.
        }
    }
}
...
void FEMSolver2D::construir_matriz_e_vetor()
{
    ...
    std::vector<std::vector<double>> dN_dxi = {
        {-1, 1, 0}, {-1, 0, 1}}; // Derivadas parciais das funções de forma em relação às coordenadas locais xi e eta.
    ...
    for (const auto &elemento : elementos)
    {
        std::vector<std::vector<double>> coords_e(
            3, std::vector<double>(2, 0.0)); // Coordenadas dos nós do elemento atual.
        for (int i = 0; i < 3; ++i)
        {
        ...
        std::vector<double> N_e =
            N(pontoDeGauss[0], pontoDeGauss[1]); // Valores das funções de forma no ponto de Gauss.
