...
void FEMSolver2D::construir_matriz_e_vetor() {
  ...
  std::vector<std::vector<double>> dN_dxi = {{-1, 1, 0}, {-1, 0, 1}}; // Derivadas das funções de forma nas coordenadas locais.
  ...
  for (size_t e = 0; e < elementos.size(); ++e) { // Itera sobre cada elemento.
    const auto &elemento = elementos[e];
    std::vector<std::vector<double>> coords_e(3, std::vector<double>(2, 0.0)); // Array para armazenar as coordenadas dos nós do elemento atual.
    for (int i = 0; i < 3; ++i) {
    ...
    // Diretriz OpenMP para garantir que a região seja crítica, impedindo que outras threads alterem a matriz simultaneamente.
    #pragma omp critical
    {
      for (int i = 0; i < 3; ++i) {
...
void FEMSolver2D::aplicar_condicoes_contorno() {
  ...
  for (size_t k = 0; k < contorno.size(); ++k) { // Itera sobre cada nó na borda.
    int idx = contorno[k];
    for (int j = 0; j < N; ++j) {
...
void FEMSolver2D::resolver_sistema() {
  ...
  for (int i = 0; i < N; ++i) {
    for (int j = 0; j < N; ++j) {
      if (K[i][j] != 0) {
        // Diretriz OpenMP para garantir que a região seja crítica, impedindo que outras threads alterem a lista de triplets simultaneamente.
        #pragma omp critical
        {
