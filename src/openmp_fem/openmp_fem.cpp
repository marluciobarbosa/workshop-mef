...
#include <omp.h>        // Inclui a biblioteca OpenMP para paralelização em múltiplos núcleos de CPU.
...
void FEMSolver2D::construir_matriz_e_vetor()
{
    ...
    std::vector<std::vector<double>> dN_dxi = {{-1, 1, 0},
                                               {-1, 0, 1}}; // Derivadas das funções de forma nas coordenadas locais.
...
#pragma omp parallel for schedule(static)
    for (size_t e = 0; e < elementos.size(); ++e)
    { // Itera sobre cada elemento.
        const auto &elemento = elementos[e];
        std::vector<std::vector<double>> coords_e(
            3, std::vector<double>(2, 0.0)); // Array para armazenar as coordenadas dos nós do elemento atual.
        for (int i = 0; i < 3; ++i)
        {
...
// Diretriz OpenMP para garantir que a região seja crítica, impedindo que outras threads alterem a matriz
// simultaneamente.
#pragma omp critical
        {
            for (int i = 0; i < 3; ++i)
            {
                f[elemento[i]] += F_e[i]; // Atualiza o vetor de forças global.
                for (int j = 0; j < 3; ++j)
                {
...
#pragma omp parallel for
    for (size_t k = 0; k < contorno.size(); ++k)
    { // Itera sobre cada nó na borda.
        int idx = contorno[k];
        for (int j = 0; j < n; ++j)
        {
...
#pragma omp parallel for
    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j < n; ++j)
        {
            if (K[i][j] != 0)
            {
// Diretriz OpenMP para garantir que a região seja crítica, impedindo que outras threads alterem a lista de triplets
// simultaneamente.
#pragma omp critical
                {
...
int main(int argc, char *argv[])
{
    ...
    start =
        std::chrono::high_resolution_clock::now(); // Marca o tempo inicial para a aplicação das condições de contorno.
