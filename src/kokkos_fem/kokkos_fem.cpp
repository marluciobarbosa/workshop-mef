...
template <typename ExecutionSpace> using MemorySpace = typename ExecutionSpace::memory_space;

// Define tipos de Views Kokkos para diferentes dimensões e espaços de execução.

template <typename ExecutionSpace>
using viewDouble2D =
    typename Kokkos::View<double **, MemorySpace<ExecutionSpace>>; // View 2D para armazenar dados do tipo double.
...
template <typename ExecutionSpace>
using viewDouble1D =
    typename Kokkos::View<double *, MemorySpace<ExecutionSpace>>; // View 1D para armazenar dados do tipo double.
...
template <typename ExecutionSpace>
using viewInt2D =
    typename Kokkos::View<int **, MemorySpace<ExecutionSpace>>; // View 2D para armazenar dados do tipo int.
...
template <typename ExecutionSpace>
using viewInt1D =
    typename Kokkos::View<double *, MemorySpace<ExecutionSpace>>; // View 1D para armazenar dados do tipo int.
...
template <typename ExecutionSpace> class FEMSolver2D
{
  public:
  ...
  private:
    ...
    static KOKKOS_INLINE_FUNCTION void N(double xi, double eta, double *N_)
    {
        N_[0] = 1 - xi - eta;
        N_[1] = xi;
        N_[2] = eta;
...
FEMSolver2D<ExecutionSpace>::FEMSolver2D(int nx, int ny)
    : nx(nx), ny(ny), n((nx + 1) * (ny + 1)), K("K", n, n), f("f", n), u("u", n), nos("nos", (nx + 1) * (ny + 1), 2),
      elementos("elementos", 2 * nx * ny, 3)
{                  // Inicializa os views Kokkos.
...
template <typename ExecutionSpace> void FEMSolver2D<ExecutionSpace>::construir_malha()
{
    ...
    Kokkos::parallel_for(
        "construir_malha", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {ny_local + 1, nx_local + 1}),
    ...
    Kokkos::parallel_for(
        "construir_elementos", ny_local * nx_local, KOKKOS_LAMBDA(int index) {
...
template <typename ExecutionSpace> void FEMSolver2D<ExecutionSpace>::construir_matriz_e_vetor()
{
    ...
    Kokkos::parallel_for(
        "construir_matriz_e_vetor", elementos_local.extent(0), KOKKOS_LAMBDA(size_t e) {
            ...
            for (int i = 0; i < 2; ++i)
            {
                for (int j = 0; j < 3; ++j)
                {
                    ...
                    {
                        dN_dxy[i][j] +=
                            invJ[i][k] * dN_dxi[k][j]; // Transformação das derivadas para coordenadas globais.
                    }
                }
            }

            double N_e[3]; // Array para armazenar os valores das funções de forma no ponto de Gauss.
            N(pontoDeGauss[0], pontoDeGauss[1], N_e);
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                F_e[i] = N_e[i] * 2 * M_PI * M_PI * sin(M_PI * x) * sin(M_PI * y) * detJ *
                         pesoDeGauss; // Cálculo do vetor de forças local.
            }

            for (int i = 0; i < 3; ++i)
            {
                Kokkos::atomic_add(
                    &f_local(elementos_local(e, i)),
                    F_e[i]); // Soma o vetor de forças local ao vetor de forças global usando operação atômica.
                for (int j = 0; j < 3; ++j)
                {
                    Kokkos::atomic_add(&K_local(elementos_local(e, i), elementos_local(e, j)),
                                       K_e[i][j]); // Soma a matriz de rigidez local à matriz de rigidez global usando
                                                   // operação atômica.
                }
            }
        });
...
template <typename ExecutionSpace> void FEMSolver2D<ExecutionSpace>::aplicar_condicoes_contorno()
{
    auto K_local = K;
    auto f_local = f;
    ...
    Kokkos::parallel_for(
        "aplicar_condicoes_contorno", n, KOKKOS_LAMBDA(const int idx) {
...
template <typename ExecutionSpace> void FEMSolver2D<ExecutionSpace>::resolver()
{
    resolver_sistema();
} // Método que chama a função de resolver o sistema linear.

template <typename ExecutionSpace> double FEMSolver2D<ExecutionSpace>::fonte(double x, double y)
{
    return 2 * M_PI * M_PI * sin(M_PI * x) * sin(M_PI * y); // Função que representa o termo fonte.
}

template <typename ExecutionSpace> void FEMSolver2D<ExecutionSpace>::resolver_sistema()
{
    Eigen::SparseMatrix<double> K_eigen(n,
                                        n); // Criação de uma matriz esparsa Eigen para armazenar a matriz de rigidez.
...
template <typename ExecutionSpace> void FEMSolver2D<ExecutionSpace>::imprimir_resultados() const
{
...
int main(int argc, char *argv[])
{
    Kokkos::initialize(argc, argv); // Inicializa a biblioteca Kokkos.
    {
        ...
        start = std::chrono::high_resolution_clock::now(); // Marca o tempo inicial para a aplicação das condições de
                                                           // contorno.
