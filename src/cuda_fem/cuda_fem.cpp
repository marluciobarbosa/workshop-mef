...
__global__ void aplicarCondicoesContornoKernel(double *d_K, double *d_f, int *d_contorno, int contornoSize, int n)
{
    ...
    if (idx < contornoSize)
    {                               // Verifica se o índice está dentro do tamanho do vetor de contorno.
...
__global__ void construirMatrizEVetorKernel(int *elementos, double *nos, double *d_K, double *d_f, int numElementos,
                                            int n, double pesoDeGauss, double *pontoDeGauss)
{

    int e = blockIdx.x * blockDim.x + threadIdx.x; // Calcula o índice global do thread.
    if (e < numElementos)
    {                        // Verifica se o índice está dentro do número de elementos.
        ...
        if (fabs(detJ) < 1e-10)
            return; // Verifica se o elemento é degenerado (detJ próximo de zero).
        ...
        double N_e[3] = {1.0 - pontoDeGauss[0] - pontoDeGauss[1], pontoDeGauss[0],
                         pontoDeGauss[1]}; // Funções de forma no ponto de Gauss.
        ...
        for (int i = 0; i < 3; ++i)
        {
            atomicAdd(&d_f[elemento[i]],
                      F_e[i]); // Soma o vetor de forças local ao vetor de forças global usando operação atômica.
            for (int j = 0; j < 3; ++j)
            {
                atomicAdd(
                    &d_K[elemento[i] * n + elemento[j]],
                    K_e[i][j]); // Soma a matriz de rigidez local à matriz de rigidez global usando operação atômica.
            }
        }
    }
}
...
FEMSolver2D::FEMSolver2D(int nx, int ny)
    : nx(nx), ny(ny), n((nx + 1) * (ny + 1)), K(n * n, 0.0), f(n, 0.0), u(n, 0.0), nos((nx + 1) * (ny + 1) * 2, 0.0),
      elementos(2 * nx * ny * 3), d_K(n * n, 0.0), d_f(n, 0.0)
{
        ...
        for (int j = 0; j < n; ++j)
        {
            if (K[i * n + j] != 0)
            {
                tripletList.push_back(
                    Eigen::Triplet<double>(i, j, K[i * n + j])); // Armazena apenas os valores não nulos.
            }
        }
