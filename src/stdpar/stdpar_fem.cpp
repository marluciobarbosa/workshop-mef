...
#include <atomic>       // Inclui a biblioteca atomic para operações atômicas.
...
#include <execution>    // Inclui a biblioteca execution para políticas de execução paralela.
...
#include <mutex>        // Inclui a biblioteca mutex para controle de concorrência em operações críticas.
...
class FEMSolver2D
{
  public:
    ...
    void construir_matriz_e_vetor();                           // Método para construir a matriz de rigidez
                                                               // e o vetor de carga.
  ...
  private:
    ...
    std::vector<std::vector<int>> elementos; // Elementos.
    std::vector<std::vector<double>> nos;    // Nós.
...
FEMSolver2D::FEMSolver2D(int nx, int ny)
    : nx(nx), ny(ny), n((nx + 1) * (ny + 1)), K(n, std::vector<double>(n, 0.0)), f(n, 0.0), u(n, 0.0)
{
    hx = 1.0 / nx; // Calcula o tamanho dos elementos na direção x.
    hy = 1.0 / ny; // Calcula o tamanho dos elementos na direção y.

    auto start = std::chrono::high_resolution_clock::now();       // Marca o tempo inicial para
                                                                  // a construção da malha.
...
{
    // Geração dos nós
    nos.resize((nx + 1) * (ny + 1), std::vector<double>(2, 0.0)); // Redimensiona o vetor de nós.
    for (int j = 0; j <= ny; ++j)
    {
        for (int i = 0; i <= nx; ++i)
        {
    ...
    elementos.reserve(2 * nx * ny); // Reserva espaço suficiente para os elementos.
    for (int j = 0; j < ny; ++j)
    {
        for (int i = 0; i < nx; ++i)
        {
            ...
            elementos.push_back({n1, n3, n2}); // Primeiro triângulo: inferior esquerdo.
            elementos.push_back({n2, n3, n4}); // Segundo triângulo: superior direito.
        }
    }
}
...
void FEMSolver2D::construir_matriz_e_vetor()
{
    ...
    const std::vector<std::vector<double>> dN_dxi = {
        {-1, 1, 0}, {-1, 0, 1}};    // Derivadas das funções de forma nas coordenadas locais.
    const double pesoDeGauss = 1.0; // Peso associado ao ponto de Gauss.
    const std::vector<double> pontoDeGauss = {1.0 / 3.0, 1.0 / 3.0}; // Ponto de Gauss no centro do triângulo.

    // Primeira parte: calcular matrizes locais
    std::vector<std::mutex> K_mutexes(n);                          // Vetor de mutexes para controle de
                                                                   // concorrência na matriz de rigidez.
    std::vector<std::atomic<double>> f_atomic(f.begin(), f.end()); // Vetor de cargas usando operações atômicas.

    // Uso de std::for_each com execução paralela
    std::for_each(
        std::execution::par, elementos.begin(), elementos.end(),
        [&](const std::vector<int> &elemento) { // Para cada elemento da malha.
            std::vector<std::vector<double>> coords_e(
                3, std::vector<double>(2, 0.0)); // Array para armazenar as coordenadas
                                                 // dos nós do elemento atual.
            for (int i = 0; i < 3; ++i)
            {
            ...
            std::vector<std::vector<double>> dN_dxy(2, std::vector<double>(3, 0.0)); // Derivadas das funções de forma
            ...
            for (int i = 0; i < 2; ++i)
            {
                for (int j = 0; j < 3; ++j)
                {
                    for (int k = 0; k < 2; ++k)
                    {
                        dN_dxy[i][j] += invJ[i][k] * dN_dxi[k][j]; // Transformação das derivadas
                                                                   // para coordenadas globais.
                    }
                }
            }

            std::vector<double> N_e = N(pontoDeGauss[0],
                                        pontoDeGauss[1]); // Calcula as funções de forma no ponto de Gauss.

            std::vector<std::vector<double>> K_e(3,
                                                 std::vector<double>(3, 0.0)); // Matriz de rigidez local do elemento.
            ...
            for (int i = 0; i < 3; ++i)
            {
                for (int j = 0; j < 3; ++j)
                {
                    ...
                    {
                        K_e[i][j] +=
                            dN_dxy[k][i] * dN_dxy[k][j] * detJ * pesoDeGauss; // Cálculo da matriz de rigidez local
            ...
            for (int i = 0; i < 3; ++i)
            {
                f_atomic[elemento[i]] += F_e[i]; // Atualiza o vetor de forças global
                                                 // usando operação atômica.
                for (int j = 0; j < 3; ++j)
                {
                    std::lock_guard<std::mutex> lock(K_mutexes[elemento[i]]); // Utiliza mutex para garantir
                                                                              // exclusividade de acesso à matriz de
                                                                              // rigidez.
    ...
    // Copiar resultados de volta para o vetor f
    for (int i = 0; i < n; ++i)
    {
        f[i] = f_atomic[i]; // Copia os valores do vetor atômico para o vetor padrão.
    }
}
...
void FEMSolver2D::aplicar_condicoes_contorno()
{
    ...
    // Uso de std::for_each com execução paralela
    std::for_each(std::execution::par, contorno.begin(), contorno.end(),
                  [&](int idx) { // Para cada nó na borda.
                      for (int j = 0; j < n; ++j)
                      {
                          K[idx][j] = 0.0; // Zera a linha correspondente na matriz de rigidez.
                      }
                      K[idx][idx] = 1.0; // Define 1 na diagonal para garantir solução única.
                      f[idx] = 0.0;      // Define o vetor de forças para 0 nesse nó
                                         // (condição de Dirichlet nula).
                  });
}
...
{
    resolver_sistema();
} // Método que chama a função de resolver o sistema linear.
...
{
    Eigen::SparseMatrix<double> K_eigen(n,
                                        n); // Criação de uma matriz esparsa Eigen
                                            // para armazenar a matriz de rigidez.
    ...
    std::vector<Eigen::Triplet<double>> tripletList; // Lista de triplets para preencher a matriz esparsa.
    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j < n; ++j)
        {
            if (K[i][j] != 0)
            {
                tripletList.push_back(
                    Eigen::Triplet<double>(i, j, K[i][j])); // Adiciona o valor não nulo na lista de triplets.
            }
        }
        f_eigen(i) = f[i]; // Copia o vetor de forças para o vetor Eigen.
    }
    K_eigen.setFromTriplets(tripletList.begin(),
                            tripletList.end()); // Cria a matriz esparsa a partir da lista de triplets.
...
int main(int argc, char *argv[])
{
    ...
    auto start = std::chrono::high_resolution_clock::now();       // Marca o tempo inicial para a
                                                                  // construção da matriz e vetor.
    ...
    // solver.imprimir_resultados(); // (Comentado) Imprime os resultados na saída
    // padrão.
