...
void FEMSolver2D::construir_matriz_e_vetor() {
  ...
  const std::vector<std::vector<double>> dN_dxi = {{-1, 1, 0}, {-1, 0, 1}}; // Derivadas das funções de forma nas coordenadas locais.
  ...
  std::vector<std::mutex> K_mutexes(N); // Vetor de mutexes para controle de concorrência na matriz de rigidez.
  ...
  std::for_each(std::execution::par, elementos.begin(), elementos.end(),
                [&](const std::vector<int> &elemento) { // Para cada elemento da malha.
                  ...
                  std::vector<std::vector<double>> J(2, std::vector<double>(
                                                              2, 0.0)); // Matriz Jacobiana do elemento.
                  for (int i = 0; i < 2; ++i) {
                    for (int j = 0; j < 2; ++j) {
                      for (int k = 0; k < 3; ++k) {
                  ...
                  std::vector<std::vector<double>> dN_dxy(
                      2, std::vector<double>(3, 0.0)); // Derivadas das funções de forma nas coordenadas globais.
                  for (int i = 0; i < 2; ++i) {
                    for (int j = 0; j < 3; ++j) {
                      for (int k = 0; k < 2; ++k) {
...
void FEMSolver2D::resolver_sistema() {
  ...
  for (int i = 0; i < N; ++i) {
    for (int j = 0; j < N; ++j) {
      if (K[i][j] != 0) {
        tripletList.push_back(Eigen::Triplet<double>(i, j, K[i][j])); // Adiciona o valor não nulo na lista de triplets.
      }
    }
    f_eigen(i) = f[i]; // Copia o vetor de forças para o vetor Eigen.
  }
  K_eigen.setFromTriplets(tripletList.begin(), tripletList.end()); // Cria a matriz esparsa a partir da lista de triplets.
